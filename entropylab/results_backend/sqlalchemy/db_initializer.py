...
class _DbInitializer:
    def __init__(self, path: str, echo=False):
        ...
        if path is not None and Path(path).suffix == ".db":
            logger.error(
                "_DbInitializer provided with path to a sqlite database. This is "
                "deprecated."
            )
            raise RuntimeError(
                ...
                "To upgrade your existing sqlite database file to an Entropy "
                "project please use the entropylab.results_backend.sqlalchemy"
                ".upgrade_db() function. * Before upgrading be sure to back up "
                "your database to a safe place *."
            )
